 ...
 * at wifi/glue/vendor_sources/rw_msg_tx.c, which existed only to include this
...
int rw_msg_send_scanu_req(SCAN_PARAM_T *scan_param)
{
	...
	if (!req) {
		bk7258_scan_diag_record(BK7258_SCAN_DIAG_REQUEST_BUILD_FAIL);
	...
	/* This is the final per-channel active/passive policy passed to LMAC.
	 * Record only aggregate counts: never export requested SSIDs, BSSID or IEs.
	 */
	{
		uint32_t active = 0;
		uint32_t passive = 0;
		...
		for (i = 0; i < req->chan_cnt; i++) {
			if (req->chan[i].flags & CHAN_NO_IR)
				passive++;
			else
				active++;
		}

		bk7258_scan_diag_request_channels(req->chan_cnt, active, passive);
	}
