 ...
 * at wifi/glue/vendor_sources/rwnx_rx.c, which existed only to include this
...
static int rwnx_rx_mgmt(struct pbuf *p, struct fhost_rx_header *rxhdr, int vif_idx)
{
	...
	if (ieee80211_is_beacon(mgmt->frame_control)) {
		bk7258_scan_diag_record(BK7258_SCAN_DIAG_HOST_MGMT_BEACON);
		...
		if (!is_wpah_queue_full()) {
			...
			bk7258_scan_diag_record(BK7258_SCAN_DIAG_HOST_MGMT_WPAQ_FORWARDED);
		} else {
			bk7258_scan_diag_record(BK7258_SCAN_DIAG_HOST_MGMT_WPAQ_DROP);
		}
		return 0;
		...
		if (ieee80211_is_probe_resp(mgmt->frame_control))
			bk7258_scan_diag_record(BK7258_SCAN_DIAG_HOST_MGMT_PROBE_RESP);
#if CONFIG_AP_PROBEREQ_CB
...
static void rwnx_rx_mgmt_any(struct pbuf *p, struct fhost_rx_header *rxhdr)
{
	...
	if (vif_idx == INVALID_VIF_IDX) {
		...
		if (dest_vif != INVALID_VIF_IDX)
			...
			bk7258_scan_diag_record(BK7258_SCAN_DIAG_HOST_MGMT_NO_STA_VIF);
	} else {
