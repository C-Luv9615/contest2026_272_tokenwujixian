/****************************************************************************
 * chips/bk7258/saradc/bk7258_saradc.c
 *
 * BK7258 SARADC implementation used by the vendor PHY calibration ABI.
 *
 * This is chip-owned because it programs shared SARADC and analog registers;
 * the Wi-Fi glue owns only the vendor-facing compatibility declarations.
 ****************************************************************************/
...
/* BK7258 defaults from authority adc_driver.c.  Calibration producers may
 * replace these through the normal authority-facing setter in a later port. */

static uint16_t g_sadc_cal_low = UINT16_C(0x55);
static uint16_t g_sadc_cal_high = UINT16_C(0x354);

/* The adapter treats this as shared SARADC state.  It belongs with the ADC,
 * rather than with the higher-level analog glue. */
...
static int bk7258_sadc_apply_config(const adc_config_t *config)
{
  ...
  modifyreg32(SADC_CTRL, SADC_CTRL_32M,
              config->src_clk == ADC_SCLK_32M ? SADC_CTRL_32M : 0);
  if (config->saturate_mode == ADC_SATURATE_MODE_NONE)
    ...
    {
      uint32_t hardware_mode;

      /* Authority adc_hal_set_saturate_mode() maps the public enum. BK7258
       * defines ADC_ACURACY_12_BIT, hence MODE_2 maps to hardware mode 2. */
      if (config->saturate_mode == ADC_SATURATE_MODE_0)
        {
          hardware_mode = 0;
        }
      else if (config->saturate_mode == ADC_SATURATE_MODE_1)
        {
          hardware_mode = 1;
        }
#if ADC_ACURACY_12_BIT
      else if (config->saturate_mode == ADC_SATURATE_MODE_2)
        {
          hardware_mode = 2;
        }
#endif
...
static int bk7258_sadc_set_voltage_divider(adc_chan_t channel,
                                           adc_vol_div_t divider)
{
  uint32_t value;
  int ret;

  if (divider == ADC_VOL_DIV_NONE)
    {
      divider = channel == ADC_0 ? ADC_VOL_DIV_5 : ADC_VOL_DIV_3;
    }
  ...
  if (channel == ADC_0)
    {
      switch (divider)
        {
          case ADC_VOL_DIV_2: value = 3; break;
          case ADC_VOL_DIV_3: value = 2; break;
          case ADC_VOL_DIV_5: value = 1; break;
          case ADC_VOL_DIV_7: value = 0; break;
  ...
  else
    {
      switch (divider)
        {
          case ADC_VOL_DIV_1: value = 3; break;
          case ADC_VOL_DIV_2: value = 2; break;
          case ADC_VOL_DIV_3: value = 1; break;
          case ADC_VOL_DIV_4: value = 0; break;
          default: return -EINVAL;
        }
    }

  /* Authority adc_hal_set_vol_div() holds ANA9.spi_latch1v around ANA5. */
  ret = bk7258_analog_update_bits(9, UINT32_C(1) << 9, UINT32_C(1) << 9);
  if (ret == OK)
    {
      ret = bk7258_analog_update_bits(5, UINT32_C(0x3) << 10, value << 10);
    }
...
bk_err_t bk_adc_init(adc_chan_t channel)
{
  ...
  /* adc_hal_init(): clear stale output before selecting the authority's
   * four-clock settling mode and the FIFO threshold. */
  (void)getreg32(SADC_DATA);
...
bk_err_t bk_adc_set_phy_cali_config(adc_config_t *config)
{
  ...
  return ret == -EINVAL ? BK_ERR_PARAM : bk7258_sadc_error(ret);
}
...
bk_err_t bk_adc_set_config(adc_config_t *config)
{
  ...
  if (ret < 0)
    {
      return ret == -EINVAL ? BK_ERR_PARAM : BK_FAIL;
    }

  ret = bk7258_sadc_set_voltage_divider(config->chan, config->vol_div);
  if (ret < 0)
    {
      return BK_ERR_PARAM;
    }
  ...
  if (config->vol_div == ADC_VOL_DIV_NONE)
    {
      config->vol_div = config->chan == ADC_0 ? ADC_VOL_DIV_5 : ADC_VOL_DIV_3;
      g_sadc.config.vol_div = config->vol_div;
    }
...
bk_err_t bk_adc_enable_bypass_clalibration(void)
{
  ...
  /* This selects raw hardware output; it never reads or writes OTP/eFuse. */
...
float saradc_calculate(UINT16 adc_val)
{
  ...
  /* Authority adc_driver.c: (raw - low) / (high - low) + 1 V. */
  if (g_sadc_cal_high <= g_sadc_cal_low)
    {
      return 0.0f;
    }

  voltage = ((float)((int32_t)adc_val - g_sadc_cal_low) /
             (float)(g_sadc_cal_high - g_sadc_cal_low)) + 1.0f;
