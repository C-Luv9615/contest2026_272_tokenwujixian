 ...
 * at wifi/glue/vendor_sources/wifi_v2.c, which existed only to include this
...
#endif
extern wifi_os_funcs_t *g_wifi_funcs;
extern wifi_os_variable_t *g_wifi_vars;
...
void bk_wlan_sta_init(network_InitTypeDef_st *inNetworkInitPara)
{
	...
	if (inNetworkInitPara) {
		...
		// CRITICAL: Clear entire array first to avoid residual data from previous SSID
...
bk_err_t bk_wifi_ap_stop(void)
{
	...
	sm_build_broadcast_deauthenticate(0xFF);
	rtos_delay_milliseconds(10);
	/* TBTT IRQ is global; keep it enabled while P2P GO is still up. */
#if CONFIG_P2P
