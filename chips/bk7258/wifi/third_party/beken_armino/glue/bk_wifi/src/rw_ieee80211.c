 ...
 * at wifi/glue/vendor_sources/rw_ieee80211.c, which existed only to include this
...
/* libwifi.a's me_strategy_mem_init() allocates this dynamically with libc
 * malloc(), then stores the result in sta_info_tab before rwnxl_init() calls
 * us.  sta_mgmt_entry_init() treats entry+0x210 as an empty co_list on its
 * first use, so the pool must be zeroed before mm_init()/sta_mgmt_init().
 *
 * The pinned prebuilt allocates g_wifi_mac_sta_max_num entries of 0x238 bytes
 * plus two extra BCMC entries.  Keep that ABI-derived extent explicit here:
 * this is a narrow pool fix, not a global malloc semantic change.
 */

extern void *sta_info_tab;
extern uint32_t g_wifi_mac_sta_max_num;

#define BK7258_STA_INFO_BYTES       UINT32_C(0x238)
#define BK7258_STA_INFO_BCMC_COUNT  UINT32_C(2)
...
UINT32 rw_ieee80211_init(void)
{
	...
	if (sta_info_tab == NULL ||
	    g_wifi_mac_sta_max_num >
	      (UINT32_MAX / BK7258_STA_INFO_BYTES) - BK7258_STA_INFO_BCMC_COUNT) {
		BK_ASSERT(0);
	}

	memset(sta_info_tab, 0,
	       (g_wifi_mac_sta_max_num + BK7258_STA_INFO_BCMC_COUNT) *
	         BK7258_STA_INFO_BYTES);
