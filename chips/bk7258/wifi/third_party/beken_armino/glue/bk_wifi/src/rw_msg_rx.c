...
UINT32 mhdr_scanu_result_ind(SCAN_RST_UPLOAD_T *scan_rst, void *msg, UINT32 len)
{
	...
	ret = BK_OK;
	result_ptr = scan_rst;
	bk7258_scan_diag_record(BK7258_SCAN_DIAG_LMAC_RESULT_IND);
	...
	if (result_ptr->scanu_num >= MAX_BSS_LIST) {
		bk7258_scan_diag_record(BK7258_SCAN_DIAG_RESULT_TABLE_FULL);
		goto scan_rst_exit;
	}
	...
	if (elmt_addr) { // adjust channel
		...
		if (rw_ieee80211_is_scan_rst_in_countrycode(chann) == 0) {
			bk7258_scan_diag_record(BK7258_SCAN_DIAG_RESULT_COUNTRY_DROP);
		...
		for (i = 0; i < result_ptr->scanu_num; i ++) {
			if (!os_memcmp(probe_rsp_ieee80211_ptr->bssid, result_ptr->res[i]->bssid, ETH_ALEN)) {
				if ((result_ptr->res[i]->on_channel == 1) || (on_channel == 0)) {
					bk7258_scan_diag_record(BK7258_SCAN_DIAG_RESULT_DUPLICATE);
					if (on_channel == 1)
	...
	if (item == NULL) {
		bk7258_scan_diag_record(BK7258_SCAN_DIAG_RESULT_ALLOC_FAIL);
		goto scan_rst_exit;
	}
	...
	if(infor_state == BK_FAIL)
		goto scan_rst_exit;
	bk7258_scan_diag_record(BK7258_SCAN_DIAG_RESULT_INSERTED);
...
void rwnx_handle_recv_msg(struct ke_msg *rx_msg)
{
	...
	case SCANU_START_CFM:
		/* scan complete */
		bk7258_scan_diag_record(BK7258_SCAN_DIAG_LMAC_COMPLETE);
		if (scan_rst_set_ptr) {
