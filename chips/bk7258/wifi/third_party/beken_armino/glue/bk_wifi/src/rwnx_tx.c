 ...
 * at wifi/glue/vendor_sources/rwnx_tx.c, which existed only to include this
...
/*
 * Correlate an EAPOL descriptor's submit and terminal confirmation without
 * changing the descriptor ABI consumed by libwifi.a.  The opaque MAC owns
 * status writeback; this probe only snapshots registered MAC IRQ counters.
 */
#define RWNX_EAPOL_CFM_PROBE_SLOTS 4

struct rwnx_eapol_cfm_probe_s
{
...
static struct rwnx_eapol_cfm_probe_s
	g_rwnx_eapol_cfm_probe[RWNX_EAPOL_CFM_PROBE_SLOTS];

static void rwnx_eapol_cfm_probe_submit(struct txdesc *txdesc)
{
	uint32_t slot;
	uint32_t empty = RWNX_EAPOL_CFM_PROBE_SLOTS;
	GLOBAL_INT_DECLARATION();

	GLOBAL_INT_DISABLE();
	for (slot = 0; slot < RWNX_EAPOL_CFM_PROBE_SLOTS; slot++)
	{
		if (g_rwnx_eapol_cfm_probe[slot].txdesc == txdesc)
		{
			empty = slot;
			break;
		}

		if (g_rwnx_eapol_cfm_probe[slot].txdesc == NULL &&
			empty == RWNX_EAPOL_CFM_PROBE_SLOTS)
		{
			empty = slot;
		}
	}

	if (empty < RWNX_EAPOL_CFM_PROBE_SLOTS)
	{
		g_rwnx_eapol_cfm_probe[empty].txdesc = txdesc;
		g_rwnx_eapol_cfm_probe[empty].isr31 = bk7258_wifi_isr_count[31];
		g_rwnx_eapol_cfm_probe[empty].isr33 = bk7258_wifi_isr_count[33];
		g_rwnx_eapol_cfm_probe[empty].isr34 = bk7258_wifi_isr_count[34];
		g_rwnx_eapol_cfm_probe[empty].isr35 = bk7258_wifi_isr_count[35];
		g_rwnx_eapol_cfm_probe[empty].isr36 = bk7258_wifi_isr_count[36];
	}
	GLOBAL_INT_RESTORE();
}

static bool rwnx_eapol_cfm_probe_take(struct txdesc *txdesc,
		struct rwnx_eapol_cfm_probe_s *probe)
{
	uint32_t slot;
	bool found = false;
	GLOBAL_INT_DECLARATION();
	...
	for (slot = 0; slot < RWNX_EAPOL_CFM_PROBE_SLOTS; slot++)
	{
		if (g_rwnx_eapol_cfm_probe[slot].txdesc == txdesc)
		{
			*probe = g_rwnx_eapol_cfm_probe[slot];
			g_rwnx_eapol_cfm_probe[slot].txdesc = NULL;
			found = true;
			break;
		}
	}
...
__ITCM_N int rwnx_start_xmit(uint8_t vif_idx, struct pbuf *p, BUS_MSG_T *msg)
{
	...
	/* This copies scalar host fields and samples only whitelisted read-only
	 * registers. It does not modify the descriptor consumed by libwifi.a. */
	if (PP_HTONS(host->ethertype) == ETH_P_PAE)
	{
		bk7258_m2_snapshot_submit(txdesc, txq->idx, host->tid,
	...
	if (PP_HTONS(host->ethertype) == ETH_P_PAE)
	{
		rwnx_eapol_cfm_probe_submit(txdesc);
	}
	// Queue skb
	if (rwnx_txq_queue_skb(skb, txq, 0, NULL))
		rwnx_hwq_process(txq->hwq);
...
void fhost_tx_cfm_push(uint8_t queue_idx, struct txdesc *txdesc)
{
	...
	if (cfm->status & TX_STATUS_DONE) {
		if (PP_HTONS(txdesc->host.ethertype) == ETH_P_PAE)
		{
			struct rwnx_eapol_cfm_probe_s probe;

			bk7258_m2_snapshot_cfm(txdesc, queue_idx, cfm->status);

			if (rwnx_eapol_cfm_probe_take(txdesc, &probe))
			{
				RWNX_LOGW("eapol cfm st=%08x q=%u tid=%u sta=%u "
						  "d31=%lu d33=%lu d34=%lu d35=%lu d36=%lu\n",
						  (unsigned)cfm->status, queue_idx,
						  txdesc->host.tid, txdesc->host.staid,
						  (unsigned long)(bk7258_wifi_isr_count[31] - probe.isr31),
						  (unsigned long)(bk7258_wifi_isr_count[33] - probe.isr33),
						  (unsigned long)(bk7258_wifi_isr_count[34] - probe.isr34),
						  (unsigned long)(bk7258_wifi_isr_count[35] - probe.isr35),
						  (unsigned long)(bk7258_wifi_isr_count[36] - probe.isr36));
			}
		}
#if 1 /* paired final confirmation probe for management and data frames */
		{
			static uint32_t final_cfm_seq;
			uint32_t n = final_cfm_seq++;
			uint8_t *dst = (uint8_t *)&txdesc->host.eth_dest_addr;
			uint8_t *src = (uint8_t *)&txdesc->host.eth_src_addr;
#if NX_UMAC_PRESENT && !NX_FULLY_HOSTED
			unsigned int saddr = (unsigned int)txdesc->host.status_desc_addr;
#else
			unsigned int saddr = 0;
#endif
#if NX_AMSDU_TX || NX_VERSION >= NX_VERSION_PACK(6, 22, 0, 0)
			unsigned int paddr = (unsigned int)txdesc->host.packet_addr[0];
			unsigned int plen = (unsigned int)txdesc->host.packet_len[0];
#else
			unsigned int paddr = (unsigned int)txdesc->host.packet_addr;
			unsigned int plen = (unsigned int)txdesc->host.packet_len;
#endif

			if (n < 48 || (n & 0x3f) == 0)
				RWNX_LOGW("txcfm seq=%u kind=%s q=%u st=%08x ack=%d done=%d "
						  "tid=%u vif=%u sta=%u fl=%08x addr=%08x len=%u "
						  "saddr=%08x eth=%04x dst="MACSTR" src="MACSTR"\n",
						  n,
						  (txdesc->host.flags & TXU_CNTRL_MGMT) ? "mgmt" : "data",
						  queue_idx, (unsigned)cfm->status,
						  !!(cfm->status & TX_STATUS_ACKNOWLEDGED),
						  !!(cfm->status & TX_STATUS_DONE),
						  txdesc->host.tid, txdesc->host.vif_idx,
						  txdesc->host.staid, (unsigned int)txdesc->host.flags,
						  paddr, plen,
						  saddr,
						  txdesc->host.ethertype, MAC2STR(dst), MAC2STR(src));
		}
#endif
